...
import { batteryList, hydrogenList } from '@/lib/data'
...
const HydrogenProgressProject = async () => {
    ...
    <>
      ...
      <div className="flex flex-col justify-center items-center min-h-[90vh]">
        <div className="w-[min(92%,1280px)]">
          <HydrogenDetail title={hydrogenList[1].title} explain={hydrogenList[1].explain} />
          {/* 소분류 컴포넌트 */}
          {/* mainCategoryIndex는 중분류를 구분하기 위해 사용 -> 방산용 Nicd:0, 산업용 Nicd:1, 동력용 Lithium:2, 에너지저장용 Lithium:3 */}
          <HydrogenItem detailInfo={data.data} mainCategoryIndex={1} />
        </div>
      </div>
    </>
  )
...
export default HydrogenProgressProject
