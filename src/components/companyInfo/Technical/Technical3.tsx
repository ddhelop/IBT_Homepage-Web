            ...
            <tbody className=" text-[#6f6f6f]">
              {researchData.map((el, index) => (
                <tr className={`text-center bg-[${el.color}]`} key={index}>
                  <td className="py-4 px-3 text-sm font-normal border-2 border-[#e2e2e2]">{el.complete} </td>
                  <td className="text-sm font-normal border-2 border-[#e2e2e2]">{el.productName}</td>
                  <td className="text-sm font-normal border-2 border-[#e2e2e2]">{el.contents}</td>
                  <td className="text-sm font-normal border-2 border-[#e2e2e2]">{el.deliveryDestination}</td>
                  <td className="text-sm font-normal border-2 border-[#e2e2e2]">{el.etc}</td>
                </tr>
              ))}
            </tbody>
          </table>
